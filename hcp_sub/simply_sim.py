...
def _plot_heatmap(corr_mat, labels, metric, output_dir):
    ...
    ax.set_xlabel("Session 1200", fontsize=10, fontweight="bold", labelpad=8)
    ax.set_ylabel("Session Retest", fontsize=10, fontweight="bold", labelpad=8)
    ...
    for metric in METRICS:
        ...
        for shift in (1, 2):
            ...
            print(f"{metric}  shift={shift}  "
                  ...
                  f"Δ={d_cross - d_within:+.3f}")
            # I think the pvals calculated here are wrong and
            # should not be used, due to the way subjects are shuffled,
            # but i leave the code here in-case
            # we need to revisit this


df = pd.read_csv("../compiled_trt_results.csv")
