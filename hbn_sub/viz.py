...
seg_sft_path = "sub-NDARKN477EBH_desc-bundles_tractography.trx"
seg_sft = aus.SegmentedSFT.fromfile(seg_sft_path, "same")
fa_file = "sub-NDARKN477EBH_model-tensor_param-fa_dwimap.nii.gz"
t1_file = "sub-NDARKN477EBH_desc-masked_T1w.nii.gz"
...
for ii, name in enumerate(seg_sft.bundle_names):
    ...
    if np.any(bundle_slice > 0):
        ...
        custom_cmap = mcolors.LinearSegmentedColormap.from_list(
            f"custom_{name}",
            [(rgb_color[0], rgb_color[1], rgb_color[2], 0.0), 
...
cfa_file = "sub-NDARKN477EBH_model-tensor_param-cfa_dwimap.nii.gz"
...
for ii in [0]:
    cfa_slice = cfa_resampled[:, a_bound:, slice_idx + ii, :]
    ...
    for jj, name in enumerate(seg_sft.bundle_names):
        bundle_slice = entire_density_map[:, a_bound:, slice_idx + ii, jj]
...
pf.add_img("tmp_cropped_cfa_slice0.png", slice(4, 8), slice(3, 6), subplot_label_pos=(0.55, 0.5), panel_label_kwargs=white_panel_label_kwargs)
...
pf.format_and_save_figure("hbn_extent.png")
